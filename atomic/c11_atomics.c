function inline u16 Atomic_Load_U16(atomic_u16* Atomic) {
	return atomic_load(&Atomic->Internal);
}

function inline void Atomic_Store_U16(atomic_u16* Atomic, u16 Value) {
	atomic_store(&Atomic->Internal, Value);
}

function inline u16 Atomic_Increment_U16(atomic_u16* Atomic) {
	return atomic_fetch_add(&Atomic->Internal, 1)+1;
}

function inline u16 Atomic_Decrement_U16(atomic_u16* Atomic) {
	return atomic_fetch_sub(&Atomic->Internal, 1)-1;
}

function inline u16 Atomic_Compare_Exchange_U16(atomic_u16* Atomic, u16 OldValue, u16 NewValue) {
...
function inline u32 Atomic_Compare_Exchange_U32(atomic_u32* Atomic, u32 OldValue, u32 NewValue) {
	atomic_compare_exchange_strong(&Atomic->Internal, &OldValue, NewValue);
	return OldValue;
}
