...
export_function void IM_Push_Rect3D_With_UV_And_Norm(v3 P0, v3 P1) {
    im_vtx_v3_n3_uv2 Vertices[] = {
        { .P = V3(P0.x, P0.y, P1.z), .N = G_ZAxis, .UV = V2(0.0f, 1.0f) },
        { .P = V3(P1.x, P0.y, P1.z), .N = G_ZAxis, .UV = V2(1.0f, 1.0f) },
        { .P = V3(P1.x, P1.y, P1.z), .N = G_ZAxis, .UV = V2(1.0f, 0.0f) },
        { .P = V3(P0.x, P1.y, P1.z), .N = G_ZAxis, .UV = V2(0.0f, 0.0f) },
        
        { .P = V3(P1.x, P0.y, P1.z), .N = G_XAxis, .UV = V2(0.0f, 1.0f) },
        { .P = V3(P1.x, P0.y, P0.z), .N = G_XAxis, .UV = V2(1.0f, 1.0f) },
    ...
    for(u32 i = 0; i < Array_Count(Vertices); i += 4) {
        u32 i0 = IM_Push(&Vertices[i+0], sizeof(im_vtx_v3_n3_uv2));
        u32 i1 = IM_Push(&Vertices[i+1], sizeof(im_vtx_v3_n3_uv2));
        u32 i2 = IM_Push(&Vertices[i+2], sizeof(im_vtx_v3_n3_uv2));
        u32 i3 = IM_Push(&Vertices[i+3], sizeof(im_vtx_v3_n3_uv2));
